...
namespace globaltraders
{
    public class UserBetsUpdateUnmatcedBets
    {
            ...
            foreach (var item in lstMarketIDS)
            {
                ...
                else
                {
                    ...
                    if (BetType == "back")
                    {
                        ...
                        {
                            MarketBook CurrentMarketProfitandloss = GetBookPosition(objMarketbook.MarketId, new List<UserBetsForAdmin>(), new List<UserBetsforAgent>(), lstUserBets);
                    ...
                    else
                    {
                        if (objMarketbook.Runners.Count == 1)
                        {
                            ...
                            if (CurrentMarketProfitandloss.Runners != null)
                            {
                                ...
                                //}
                                //var selectedrunner = CurrentMarketProfitandloss.Runners.Where(item2 => item2.SelectionId == selectionID).FirstOrDefault();
                                //if (selectedrunner != null)
                                //{
                                //    ProfitandLoss = selectedrunner.ProfitandLoss;
                                //}
                    ...
                    if (objMarketbook != null)
                    {
                        ...
                        else
                        {
                            if (objMarketbook.Runners.Count == 1)
                            {
                                ...
                                MarketBook CurrentMarketProfitandloss = GetBookPosition(objMarketbook.MarketId, new List<UserBetsForAdmin>(), lstUserBets, new List<UserBets>());
            ...
            try
            {
                ...
                else
                {
                    if (marketbook.Runners.Count == 1)
                    {
                        ...
                        MarketBook CurrentMarketProfitandloss = GetBookPosition(marketbook.MarketId, new List<UserBetsForAdmin>(), lstUserBet, new List<UserBets>());
                    ...
                    if (objMarketbook != null)
                    {
                            ...
                            foreach (var userid in lstUsers)
                            {
                                ...
                                if (lstuserbet.Count > 0)
                                {
                                    ...
                                    foreach (var runner in objMarketbook.Runners)
                                    {
                                        ...
                                        long profitorloss = Convert.ToInt64(objMarketbook.DebitCredit.Where(item2 => item2.SelectionID == runner.SelectionId).Sum(item2 => item2.Debit));
                                        long loss = Convert.ToInt64(objMarketbook.DebitCredit.Where(item2 => item2.SelectionID == runner.SelectionId).Sum(item2 => item2.Credit));
                                        decimal adminrate = TransferAdminAmount == false ? 100 - Convert.ToDecimal(agentrate) : adminrateacc;
                        ...
                        else
                        {
                            if (objMarketbook.Runners.Count == 1)
                            {
                                ...
                                MarketBook CurrentMarketProfitandloss = GetBookPosition(objMarketbook.MarketId, lstUserBets, new List<UserBetsforAgent>(), new List<UserBets>());
                                ...
                                foreach (var userid in lstUsers)
                                {
                                    ...
                                    if (lstuserbet.Count > 0)
                                    {
                                        ...
                                        foreach (var runner in objMarketbook.Runners)
                                        {
                                            ...
                                            {
                                                profit = ((adminrateacc) / 100) * profitorloss;
                                            }
            ...
            try
            {
                ...
                else
                {
                    if (marketbook.Runners.Count == 1)
                    {
                        ...
                        MarketBook CurrentMarketProfitandloss = GetBookPosition(marketbook.MarketId, lstUserBet, new List<UserBetsforAgent>(), new List<UserBets>());
                                ...
                                foreach (var userid in lstUsers)
                                {
                                    ...
                                    if (lstuserbet.Count > 0)
                                    {
                                        ...
                                        foreach (var runner in objMarketbook.Runners)
                                        {
                                            ...
                                            decimal profit = TransferAdminAmount == false ? ((100 - agentrate) / 100) * profitorloss : ((adminrateacc) / 100) * profitorloss;
                                            runner.ProfitandLoss += Convert.ToInt64(-1 * profit);
        ...
        public List<LiabalitybyMarket> GetLiabalityofAdminbyMarketsFancy(List<UserBetsForAdmin> lstUserBet)
        {
            try
            {
                long OverAllLiabality = 0;
                    ...
                    if (objMarketbook != null)
                    {
                        ...
                        if (marketbooknames.Count > 0)
                        {
                            ...
                            objMarketbook.MarketBookName = marketbookname;
                                ...
                                {
                                    List<UserBetsForAdmin> lstuserbet = lstUserBets.Where(item2 => item2.UserID == Convert.ToInt32(userid.UserID) && item2.MarketBookID == objMarketbook.MarketId).ToList();
                                    objMarketbook.DebitCredit = ceckProfitandLossAdminFancy(objMarketbook, lstuserbet);
                                    if (lstuserbet.Count > 0)
                                    {
                                        decimal agentrate = Convert.ToDecimal(lstuserbet[0].AgentRate);
                                        bool TransferAdminAmount = lstuserbet[0].TransferAdmin;
                ...
                foreach (var marketbook in lstmarketbooks)
                {
                    ...
                    var result1 = lstUserBets.Where(x => x.MarketBookname == marketbook.MarketId).FirstOrDefault();
                    ...
                    objMarketLiabality.MarketBookID = result.ToString();
                        objMarketLiabality.MarketBookName = marketbook.MarketId;
                        objMarketLiabality.Liabality = LastProfitandLoss;
                        lstLiabalitybyMarket.Add(objMarketLiabality);
                        OverAllLiabality += LastProfitandLoss;
                        LastProfitandLoss = 0;
        ...
        public List<ExternalAPI.TO.DebitCredit> ceckProfitandLossAdminFancy(ExternalAPI.TO.MarketBook marketbookstatus, List<UserBetsForAdmin> lstUserBets)
        {
            try
            {
        ...
        public MarketBook GetBookPosition(string marketBookID, List<UserBetsForAdmin> CurrentAdminBets, List<UserBetsforAgent> CurrentAgentBets, List<UserBets> CurrentUserBets)
        {
            ...
            if (LoggedinUserDetail.GetUserTypeID() == 1)
            {
                ...
                if (lstCurrentBetsAdmin.Count > 0)
                {
                    ...
                    foreach (var userid in lstUsers)
                    {
                        ...
                        int transadpercen = ((100 - TransferAdminpercentage) / 100);
