                        ...
                        if (lstrunners.Count == 1)
                        {
                            ...
                            if (lstcurrentmarketbets.Count > 0)
                            {
                              ...
                              // lstcurrentmarketbets.Where(item => item.BetType == "back" ).ToList().Sum(item => Convert.ToInt64(item.Amount));
                                ExternalAPI.TO.MarketBook CurrentMarketProfitandloss = objUserBets.GetBookPosition(marketbookID, new List<UserBetsForAdmin>(), new List<UserBetsforAgent>(), lstUserBets);
