...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceWinner" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceWinner.svc or BettingServiceWinner.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceWinner : IBettingServiceWinner
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedWinner().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrWinner = client.DownloadString(url);
                    }
            ...
            try
            {
                ...
                else
                {
                    if (APIConfig.GetCricketDataFrom == "BP")
                    {
                       // string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedWinner().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.MarketCatalogueName == "Winner").Select(item => item.MarketCatalogueID).ToList();
