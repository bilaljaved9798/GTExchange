...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceSoccer" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceSoccer.svc or BettingServiceSoccer.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceSoccer : IBettingServiceSoccer
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedSoccer().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrSoccer = client.DownloadString(url);
                    }
            ...
            try
            {
                ...
                else
                {
                    if (APIConfig.GetCricketDataFrom == "BP")
                    {
                       // string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedSoccer().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "1").Select(item => item.MarketCatalogueID).ToList();
                    ...
                    else
                    {
                       ...
                       // string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedSoccer().FirstOrDefault();
                        ...
                        if (marketIDs != "")
                        {
                            ...
                            objGetData.GetCurrentMarketBooksOther123(Password, marketIDs, ws1,1,false);
