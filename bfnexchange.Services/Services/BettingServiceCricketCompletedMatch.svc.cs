...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceCricketCompletedMatch" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceCricketCompletedMatch.svc or BettingServiceCricketCompletedMatch.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceCricketCompletedMatch : IBettingServiceCricketCompletedMatch
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedCricketCompletedMatch().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrCricketCompletedMatch = client.DownloadString(url);
                    }
            ...
            try
            {
                ...
                if (APIConfig.GetCricketDataFrom == "Live" && ConfigurationManager.AppSettings["GetDataFromLiveOtherEventTypes"].ToString()=="1")
                {
                    ...
                    if (marketIDs != "")
                    {
                        ...
                        objGetData.GetCurrentMarketBooksOther123(Password, marketIDs, ws1, 4, false);
                ...
                else
                {
                    if (APIConfig.GetCricketDataFrom == "BP")
                    {
                        //  string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedCricketCompletedMatch().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "4" && (item.MarketCatalogueName == "Completed Match" || item.MarketCatalogueName == "Tied Match" || item.MarketCatalogueName == "To Win the Toss" || item.MarketCatalogueName.Contains("Innings Runs"))).Select(item => item.MarketCatalogueID).ToList();
