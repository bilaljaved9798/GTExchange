...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceGrayHoundPlace" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceGrayHoundPlace.svc or BettingServiceGrayHoundPlace.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceGrayHoundPlace : IBettingServiceGrayHoundPlace
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedGrayHoundRacePlace().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrGrayHoundPlace = client.DownloadString(url);
                    }
            ...
            try
            {
                ...
                else
                {
                    if (APIConfig.GetCricketDataFrom == "BP")
                    {
                        //string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedGrayHoundRacePlace().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "4339" && (item.MarketCatalogueName == "To Be Placed")).Select(item => item.MarketCatalogueID).ToList();
                        ...
                        if (marketIDs != "")
                        {
                            ...
                            //objGetData.GetCurrentMarketBookCricket(Password, marketIDs,false);
                    ...
                    else
                    {
                        ...
                        ws1.Url = APIConfig.URLsData.Where(item => item.EventType == "GreyHound Racing").FirstOrDefault().URLForData;
                       // string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedGrayHoundRacePlace().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "4339" && (item.MarketCatalogueName == "To Be Placed")).Select(item => item.MarketCatalogueID).ToList();
                        if (lstofobjects.Count > 0)
                        ...
                        if (marketIDs != "")
                        {
                            ...
                            GetDataFromOtherSource objGetData = new GetDataFromOtherSource();
                            objGetData.GetCurrentMarketBooksOther123(Password, marketIDs, ws1,13,false);
