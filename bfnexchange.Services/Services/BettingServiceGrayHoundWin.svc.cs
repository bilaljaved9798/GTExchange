...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceGrayHoundWin" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceGrayHoundWin.svc or BettingServiceGrayHoundWin.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceGrayHoundWin : IBettingServiceGrayHoundWin
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedGrayHoundRaceWin().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrGrayHoundWin = client.DownloadString(url);
                    }
