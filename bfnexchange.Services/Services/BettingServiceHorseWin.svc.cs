...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceHorseWin" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceHorseWin.svc or BettingServiceHorseWin.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceHorseWin : IBettingServiceHorseWin
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedHorseRaceWin().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrHorseRaceWin = client.DownloadString(url);
                    }
