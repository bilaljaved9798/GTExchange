...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "BettingServiceHorseRacePlace" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select BettingServiceHorseRacePlace.svc or BettingServiceHorseRacePlace.svc.cs at the Solution Explorer and start debugging.
    [ServiceBehavior(ConcurrencyMode = ConcurrencyMode.Multiple, InstanceContextMode = InstanceContextMode.PerCall)]
    [AspNetCompatibilityRequirements(
        RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class BettingServiceHorseRacePlace : IBettingServiceHorseRacePlace
    {
        ...
        {
            string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedHorseRacePlace().FirstOrDefault();
            if (marketIDs != "")
            {
                ...
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        APIConfigforResults.JsonResultsArrHorseRacePlace = client.DownloadString(url);
                    }
            ...
            try
            {
                ...
                else
                {
                    if (APIConfig.GetCricketDataFrom == "BP")
                    {
                        //string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedHorseRacePlace().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "7" && (item.MarketCatalogueName == "To Be Placed")).Select(item => item.MarketCatalogueID).ToList();
                    ...
                    else
                    {
                        ...
                        ws1.Url = APIConfig.URLsData.Where(item => item.EventType == "Horse Racing").FirstOrDefault().URLForData;
                       // string marketIDs = dbEntities.SP_UserMarket_GetDistinctMarketsOpenedHorseRacePlace().FirstOrDefault();
                        string marketIDs = "";
                        var lstofobjects = OpenMarkets.OpenMarketlst.ToList().Where(item => item.EventTypeID == "7" && (item.MarketCatalogueName == "To Be Placed")).Select(item => item.MarketCatalogueID).ToList();
                        if (lstofobjects.Count > 0)
