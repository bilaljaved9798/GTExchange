...
{
    public class SmallFigController : Controller
    {
        ...
        public PartialViewResult GetFancyMarketSFig(string eventID, bool Bools)
        {
            if (Bools == true)
            {
                ...
                if (KJvmarket.Count > 0)
                {
                    ...
                    MarketBook.KJMarkets = KJvmarket.Where(item => item.EventName == "SmallFig").ToList();
                ...
                if (MarketBook.KJMarkets != null)
                {
                    foreach (var bfobject in MarketBook.KJMarkets)
                    {
                        try
                        {
                            LastloadedLinMarkets1.Add(ConvertJsontoMarketObjectBFNewSource("3121", bfobject.MarketCatalogueID, bfobject.MarketCatalogueName, bfobject.BettingAllowed));
                        }
                    ...
                    return PartialView("FancySFigMarketBook", LastloadedLinMarkets1);
                }
            }
        ...
        public string LoadMarketSFig(string eventID)
        {
            List<ExternalAPI.TO.SP_UserMarket_GetDistinctKJMarketsbyEventID_Result> KJvmarket = (List<ExternalAPI.TO.SP_UserMarket_GetDistinctKJMarketsbyEventID_Result>)Session["Allmarkets"];
